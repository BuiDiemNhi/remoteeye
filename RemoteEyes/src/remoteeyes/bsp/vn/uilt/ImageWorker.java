...
import remoteeyes.bsp.vn.BuildConfig;
...
public abstract class ImageWorker {
	...
	public void loadGalleryImage(Object data, RecyclingImageView imageView,
			ProgressBar progressBarGallery, TextView txtLabel) {
		if (data == null) {
			return;
		}
	...
	class mAsynctask extends AsyncTask<String, Void, String> {
		...
		public mAsynctask(RecyclingImageView imageView,
				ProgressBar progressBarGallery, TextView txtLabel) {
			this.imageView = imageView;
			this.progressBarGallery = progressBarGallery;
		...
		protected String doInBackground(String... params) {
			...
			while (imageView.getDrawable() == null && i <= 11) {
				try {
					Thread.sleep(1500);
				} catch (InterruptedException e) {
					e.printStackTrace();
		...
		protected void onPostExecute(String result) {
			...
			if (i >= 12) {
				...
				txtLabel.setText("Không thể tải hình. Vui lòng kiểm tra lại kết nối Internet");
			} else {
				try {
